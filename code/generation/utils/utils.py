...
def grid_building(
    parkimeters, slots, grid_size, scenario, mapping_dict, time_start, time_end
):
    """
    ...
    for key in parkimeters.keys():
        parkimeters_list.append(
            parkimeters[key]["data"].loc[time_start:time_end].values
        )
    ...
    df_parkimeters_scaled = pd.DataFrame(
        scaler_parkimeters.fit_transform(parkimeters),
        columns=parkimeters.columns,
        index=parkimeters.index,
    )
    for key in parkimeters.keys():
    ...
    df_slots_scaled = pd.DataFrame(
        scaler_slots.fit_transform(slots), columns=slots.columns, index=slots.index
    )
    for key in slots.keys():
    ...
    if scenario == "2nd":
        ...
        for key, value in dict_road.items():
            ...
            strade_dict[key] = strade_dict[key].apply(
    ...
    if scenario == "1st" or scenario == "3rd":
        ...
        for key in parkimeters.keys():
            matrix[indices_parkimeter[key][0], indices_parkimeter[key][1], 1, :] = (
                parkimeters[key]["data"].values
            )
        ...
        for key in strade_dict.keys():
            for key2 in slots.keys():
                if slots[key2]["id_strada"] == key:
                    road_matrix[
                        indices_slots[key2][0], indices_slots[key2][1], 0, :
                    ] = strade_dict[key].values
